...
def keccak_f23(s):
    """Keccak-f[1600] running rounds 1..23 (skipping round 0)"""
    a = list(s)
    for r in range(1, 24):
        # Theta
...
def solve_pow(challenge_hex: str, salt: str, expire_at: int, difficulty: int) -> int:
    """
    Find nonce n in [0, difficulty) such that:
    ...
    raise ValueError(f"No solution found in {difficulty} iterations")


def build_pow_header(challenge: dict, answer: int) -> str:
    """
    Build x-ds-pow-response header = base64(JSON({...}))
    """
    payload = {
...
def solve_challenge(challenge: dict) -> str:
    """
    End-to-end: receive challenge dict → return x-ds-pow-response header value
    """
